...
public class RGBHyperMaze : MonoBehaviour {
    ...
    private void Awake()
    {
        ...
        for(int i = 0; i < 16; i++)
        {
            //verticesSel[i].OnInteract += delegate () { PressVertex(i); return false; };
            verticesSel[i].OnInteract = PressVertex(i);
        }

        /*for(int i = 0; i < 16; i++)
        {
            fDCoords[i] = new Vector4(1f,1f,1f,1f);
        }*/

        for (int ID = 0; ID < balls.childCount; ID++)
        {
        ...
        float hue = rnd.Next(100);
        spinEdgeColor = Color.HSVToRGB(hue/100,1f,0.7f,true);
        spinVertexColor = Color.HSVToRGB(hue/100,1f,0.5f,true);

        for(int i = 0; i < 32; i++)
        {
            edgesCol[i].material.color = spinEdgeColor;
        }
        for(int i = 0; i < 16; i++)
        {
            verticesCol[i].material.color = spinVertexColor;
        }
        
        List<int> possibleRots = new List<int>();
        for(int i = 0; i < 12; i++)
        {
            possibleRots.Add(i);
        }
        ...
        {
            passableTranslated.Add(edgeTranslator(passable[i]));
        }
        ...
        for(int i = 0; i < 32; i++)
        {
            rotationLogic[i,0] = true;
        }
        for (int i = 0; i < passableTranslated.Count; i++)
        {
            rotationLogic[passableTranslated[i],0] = false;
            //edgesCol[passableTranslated[i]].material.color = new Color(0f, 1f, 0f);
        }
        
        List<int> possibleAppRots = new List<int>();
        for(int i = 0; i < 5; i++)
        {
            possibleAppRots.Add(i);
        }
        for(int i = 0; i < 4; i++){
	...
	void Update () {
        ...
        else if(!start || step % 200 >= 100 || step % 200 < 50){
            displayText.text = "----";
            step += 1;
        ...
        {
            if(inputState == true && visualState == false){
                if(transitionProg == 0){
                    for(int i = 0; i < 32; i++)
                    {
                        if(i < 16)
            ...
            if(inputState == false && visualState == true){
                if(transitionProg == 0){
                    for(int i = 0; i < 32; i++)
                    {
                        if(i < 16)
            ...
            if(inputState == true && visualState == true)
            {
                for(int i = 0; i < 16; i++)
                {
                    if(i == goalVertex)
            ...
            else if(inputState == false && visualState == false && !moduleSolved)
            {
    ...
    private KMSelectable.OnInteractHandler PressVertex(int id)
    {
        return delegate{
            verticesSel[id].AddInteractionPunch(0.2f);
            audio.PlayGameSoundAtTransform(KMSoundOverride.SoundEffect.ButtonPress, transform);
            if(visualState && inputState)
            {
                int passingEdge = -1;
                for(int i = 0; i < 32; i++){
                ...
                else
                {
                    ...
                    {
                        Debug.LogFormat("[RGB Hypermaze #{0}] Attempted to move across a wall. Strike!", moduleId);
                    ...
                    {
                        currentVertex = id;
                        Debug.LogFormat("[RGB Hypermaze #{0}] Successfully moved current vertex.", moduleId);
    ...
    public void rotate(int progress, int rotId)
    {
        if(progress == 150)
        {
            progress = 0;
    ...
    public Vector3 convert4DTo3D(double x, double y, double z, double w)
    {
    ...
    public void BackTracker(int x, int y, int z, int w)
    {
    ...
    public void ColorPuzzler()
    {
        ...
        while(reset)
        {
            ...
            for(int i = 0; i < 32; i++)
            {
                List<int> possibleCol = new List<int>();/*operatorID[i]) (inside switch()), change back later*/
                switch(operatorID[i]) 
                {
                    case 0:
                        for(int j = 0; j < 3; j++)
                        ...
                        {
                            reset = true;
                            //Debug.Log("are you bad? do you fucking suck?");
                        }
                        else
                        {
                ...
                if(reset)
                {
                    //Debug.Log("maybe i am bad. maybe i fucking suck.");
            ...
            {
                Debug.Log("BAD YOU'RE BAD YOU FUCKING SUCK");
                break;
            }*/
        }
    }
    ...
    public int edgeTranslator(int[] dims)
    {
    ...
    public void performRotation(int id, int i)
    {
        switch(id)
        {
            case 0:
    ...
    public void logRots(int[] rots, int[] order)
    {
        string rotString = "";
    ...
    public void logMazes(Color[] verts, Color[] edges)
    {
        ...
        Debug.LogFormat("[RGB Hypermaze #{0}] The colors are as follows:\n|    {6}—————{22}—————{7}  <-Bottom      {14}—————{34}—————{15}\n|   /|          /|     Cube      /|          /|\n|  {26} |         {27} |              {38} |         {39} |\n| /  {21}        /  {23}     Top     /  {33}        /  {35}\n|{2}———|—{18}—————{3}   |    Cube->  {10}———|—{30}—————{11}   |\n||   |       |   |            |   |       |   |\n||   {5}—————{24}—————{8}            |   {13}—————{36}—————{16}\n|{17}  /        {19}  /             {29}  /        {31}  / \n|| {25}         | {28}              | {37}         | {40}  \n||/          |/           |   |/          |/   \n|{1}—————{20}—————{4}    Y-Edges v   {9}—————{32}—————{12}    \n|                                              \n|                  {46}          {47}                \n|                                              \n|             {42}          {43}                     \n|                                              \n|                  {45}          {48}                \n|                                              \n|             {41}          {44}                     \n|                                              ",
                            moduleId,
        ...
        Debug.LogFormat("[RGB Hypermaze #{0}] The colored cube's maze is as follows, where P = passage and W = wall:\n|    *—————{6}—————*  <-Bottom      *—————{18}—————*\n|   /|          /|     Cube      /|          /|\n|  {10} |         {11} |              {22} |         {23} |\n| /  {5}        /  {7}     Top     /  {17}        /  {19}\n|*———|—{2}—————*   |    Cube->  *———|—{14}—————*   |\n||   |       |   |            |   |       |   |\n||   *—————{8}—————*            |   *—————{20}—————*\n|{1}  /        {3}  /             {13}  /        {15}  / \n|| {9}         | {12}              | {21}         | {24}  \n||/          |/           |   |/          |/   \n|*—————{4}—————*    Y-Edges v   *—————{16}—————*    \n|                                              \n|                  {30}          {31}                \n|                                              \n|             {26}          {27}                     \n|                                              \n|                  {29}          {32}                \n|                                              \n|             {25}          {28}                     \n|                                              ",
                            moduleId,
        ...
        Debug.LogFormat("[RGB Hypermaze #{0}] The input cube's maze is as follows, where P = passage and W = wall:\n|    *—————{6}—————*  <-Bottom      *—————{18}—————*\n|   /|          /|     Cube      /|          /|\n|  {10} |         {11} |              {22} |         {23} |\n| /  {5}        /  {7}     Top     /  {17}        /  {19}\n|*———|—{2}—————*   |    Cube->  *———|—{14}—————*   |\n||   |       |   |            |   |       |   |\n||   *—————{8}—————*            |   *—————{20}—————*\n|{1}  /        {3}  /             {13}  /        {15}  / \n|| {9}         | {12}              | {21}         | {24}  \n||/          |/           |   |/          |/   \n|*—————{4}—————*    Y-Edges v   *—————{16}—————*    \n|                                              \n|                  {30}          {31}                \n|                                              \n|             {26}          {27}                     \n|                                              \n|                  {29}          {32}                \n|                                              \n|             {25}          {28}                     \n|                                              ",
                            moduleId,
    ...
    public void solveAnim(int progress)
    {
